...
type MySqlFactory interface {
	OpenConnection(connString string, config *config.Config)
	Close() error
	GetDB() (*DB, error)
...
func (r *DB) OpenConnection(connString string, config *config.Config) {
...
func open(driverName, connString string, config *config.Config) (*DB, error) {
